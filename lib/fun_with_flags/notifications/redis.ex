if Code.ensure_loaded?(Redix.PubSub) do
  defmodule FunWithFlags.Notifications.Redis do
    ...
    def start_link(redis_conn_config) do
      GenServer.start_link(__MODULE__, {redis_conn_config, Config.build_unique_id()},
        name: __MODULE__
      )
    end
    ...
    def handle_info(
          {:redix_pubsub, _from, ref, :subscribed, %{channel: @channel}},
          state = {_, ref}
        ) do
      {:noreply, state}
    end
    ...
    def handle_info(
          {:redix_pubsub, _from, ref, :unsubscribed, %{channel: @channel}},
          state = {_, ref}
        ) do
      {:noreply, state}
    end
    ...
    def handle_info({:redix_pubsub, _from, ref, :disconnected, %{error: error}}, state = {_, ref}) do
      Logger.error(
        "FunWithFlags: Redis pub-sub connection interrupted, reason: '#{Redix.ConnectionError.message(error)}'."
      )
    ...
    #
    def handle_info(
          {:redix_pubsub, _from, ref, :message, %{channel: @channel, payload: msg}},
          state = {unique_id, ref}
        ) do
