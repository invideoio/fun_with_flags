if Code.ensure_loaded?(Phoenix.PubSub) do
  defmodule FunWithFlags.Notifications.PhoenixPubSub do
    ...
    def publish_cache_flush do
      ...
      Task.start(fn ->
        Phoenix.PubSub.broadcast!(client(), @channel, {:fwf_changes, {:cache_flush, unique_id()}})
      end)
    end
      ...
      try do
        case Phoenix.PubSub.subscribe(client(), @channel) do
          :ok ->
            # All good
            Logger.debug(fn ->
              "FunWithFlags: Connected to Phoenix.PubSub process #{inspect(client())}"
            end)
          ...
          {:error, reason} ->
            ...
            Logger.debug(fn ->
              "FunWithFlags: Cannot subscribe to Phoenix.PubSub process #{inspect(client())} ({:error, #{inspect(reason)}})."
            end)
    ...
    #
    def handle_call(
          {:test_helper_set_subscription_status, new_subscription_status},
          _from,
          {unique_id, _current_subscription_status}
        ) do
      {:reply, :ok, {unique_id, new_subscription_status}}
    end

    def handle_info(
          {:fwf_changes, {:updated, _name, unique_id}},
          state = {unique_id, _subscription_status}
        ) do
    ...
    def handle_info({:subscribe_retry, attempt}, state = {unique_id, _subscription_status}) do
      Logger.debug(fn ->
        "FunWithFlags: retrying to subscribe to Phoenix.PubSub, attempt #{attempt}."
      end)

      case subscribe(attempt) do
        :subscribed ->
          Logger.debug(fn ->
            "FunWithFlags: updating Phoenix.PubSub's subscription status to :subscribed."
          end)
